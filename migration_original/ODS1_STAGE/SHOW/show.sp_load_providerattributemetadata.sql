        ...
        select
            ...
            '<prov>' || listagg(
                '<de>' || iff(
                    ...
                    '<elem>' || CTE_updates.dataelement || '</elem>',
                    ''
                ) || iff(
                    CTE_updates.sourcecode is not null,
                    '<src>' || CTE_updates.sourcecode || '</src>',
                    ''
                ) || iff(
                    CTE_updates.lastupdatedate is not null,
                    '<upd>' || CTE_updates.lastupdatedate || '</upd>',
                    ''
                ) || '</de>',
                ''
            ) || '</prov>'
...
select
    p.providerid,
    p.providercode,
    to_variant(parse_xml(UpdateTrackingXML)) as UpdateTrackingXML,
    u.updatedby,
