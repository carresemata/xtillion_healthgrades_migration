...
class SnowflakeTableValidator(Validator):
    ...
    def aggregate_validation(self, table_name_sql_server, table_name_snowflake):
        ...
        for col_name in col_names:
            ...
            try:
                ...
                print(total_distincts_snowflake)
                print(total_rows_snowflake)
                new_row = {'Column_Name': col_name,
                   'Total_Distincts_Snowflake': int(total_distincts_snowflake)*100/int(total_rows_snowflake),
                   'Total_Distincts_SQLServer': int(total_distincts_sql_server)*100/int(total_rows_sql_server)}
    ...
    def generate_report_markdown(self, table_name_sql_server, table_name_snowflake, match_ids, sample_size) -> str:
        ...
        with open('results/'+output_file, "w") as file:
            file.write(report)
...
if __name__ == "__main__":
    ...
    # schemas = os.listdir("ODS1Stage/tables")
    # for schema in schemas:
    #     tables = os.listdir(f"ODS1Stage/tables/{schema}")
    #     if schema != 'Show':
    #         for table in tables:
    #             table_name_sql_server = f"{schema}.{table}".upper()
    #             table_name_snowflake = f"{schema}.{table}".upper()
    #             print(table_name_sql_server)
        ############################### Example Usage ###############################
    table_name_sql_server = "BASE.CLIENTPRODUCTENTITYTODISPLAYPARTNERPHONE"  
    table_name_snowflake = "BASE.CLIENTPRODUCTENTITYTODISPLAYPARTNERPHONE"  # in case it's a different schema or has different naming convention 
    match_ids = ["CLIENTPRODUCTENTITYTODISPLAYPARTNERPHONEID"] # we should remember to never use IDs since we are creating them in runtime
