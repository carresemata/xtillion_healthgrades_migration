-- mid.spuprovidermalpracticerefresh
if @IsProviderDeltaProcessing = 0 begin
	...
	--build a temp table with the same structure as the Mid.ProviderMalpractice
		begin try drop table #ProviderMalpractice end try begin catch end catch
		select top 0 *
		into #ProviderMalpractice
		from Mid.ProviderMalpractice
		
		alter table #ProviderMalpractice
	...
	--populate the temp table with data from Base schemas
		insert into #ProviderMalpractice 
			(
				ProviderMalpracticeID, ProviderID,MalpracticeClaimTypeCode,MalpracticeClaimTypeDescription,ClaimNumber,ClaimDate,ClaimYear,ClaimAmount,
				Complaint,IncidentDate,ClosedDate, ClaimState, ClaimStateFull, LicenseNumber, ReportDate
			)
		select distinct a.ProviderMalpracticeID, a.ProviderID, b.MalpracticeClaimTypeCode, b.MalpracticeClaimTypeDescription, a.ClaimNumber, a.ClaimDate, ClaimYear, 
			case when a.ClaimAmount is not null then cast(a.ClaimAmount as varchar(50)) else a.MalpracticeClaimRange end,				
			a.Complaint, a.IncidentDate, a.ClosedDate, a.ClaimState, c.StateName as ClaimStateFull, a.LicenseNumber, a.ReportDate
		...
		inner join Base.ProviderMalpractice as a with (nolock) on a.ProviderID = pb.ProviderID
		inner join Base.MalpracticeClaimType as b with (nolock) on a.MalpracticeClaimTypeID = b.MalpracticeClaimTypeID
		INNER JOIN Base.MalpracticeState AS ms WITH (nolock) ON a.ClaimState = ms.State AND ISNULL(ms.Active,1) = 1
		left join Base.State c on a.ClaimState = c.State
	...
	*/
		--ActionCode Insert
			...
			set a.ActionCode = 1
			--select *
			from #ProviderMalpractice a
			left join Mid.ProviderMalpractice b on (a.ProviderMalpracticeID = b.ProviderMalpracticeID)
			where b.ProviderMalpracticeID is null
		...
		--ActionCode Update
			...
			where id = object_id('TempDB..#ProviderMalpractice')
			and name not in ('ProviderMalpracticeID'/*PK*/, 'ActionCode')
			...
			--build the sql statement with dynamic sql to check if we need to update any columns
				...
				set @sql = 'update a'+@newline+ 
						   ...
						   '--select *'+@newline+
						   'from #ProviderMalpractice a'+@newline+
						   'join Mid.ProviderMalpractice b with (nolock) on (a.ProviderMalpracticeID = b.ProviderMalpracticeID)'+@newline+
						   'where '
	...
	*/
		...
		--ActionCode = 1 (Inserts)
			...
			set @sqlInsert = 
			'insert into Mid.ProviderMalpractice ('+@columnListInsert+')
			select '+@columnListInsert+' from #ProviderMalpractice where ActionCode = 1'
		...
		--ActionCode = 2 (Updates)	
			...
			set @sqlUpdatesClause = '--select *'+@newlineUpdates+
							  'from Mid.ProviderMalpractice a with (nolock)'+@newlineUpdates+
							  'join #ProviderMalpractice b on (a.ProviderMalpracticeID = b.ProviderMalpracticeID)'+@newlineUpdates+
		...
		--ActionCode = N (Deletes)
			delete a
			--select *
			from Mid.ProviderMalpractice a with (nolock)
            inner join #ProviderBatch as pb on pb.ProviderID = a.ProviderID	
			left join #ProviderMalpractice b on (a.ProviderMalpracticeID = b.ProviderMalpracticeID)
			where b.ProviderMalpracticeID is null