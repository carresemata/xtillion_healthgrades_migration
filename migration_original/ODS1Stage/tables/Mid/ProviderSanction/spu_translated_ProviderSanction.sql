CREATE or REPLACE PROCEDURE ODS1_STAGE_TEAM.MID.SP_LOAD_PROVIDERSANCTION(is_full BOOLEAN)
    RETURNS STRING
    LANGUAGE SQL
    EXECUTE as CALLER
    as  
...
merge_statement := ' merge into mid.providersanction as target using 
                   ...
                   on source.providersanctionid = target.providersanctionid and source.providerid = target.providerid
...
if (is_full) then
    truncate table Mid.ProviderSanction;
end if; 
